...
Two later additions (September 2026), both documented in the memo `../memo-infty-homology/infty-homology-memo.pdf`: `homology/` holds the homology-level check of Theorem 1.3 (checks C10–C12, see its own README: integral homology of `P(n)` and the `Lie(n)⊗sgn` character for `n ≤ 6`; the maps induced on top homology by the two operad structures of Theorem 1.3 agree for every composition profile with `k ≤ 6`; the resulting homology operad satisfies the operad axioms and is the operadic suspension of Lie), and `infty/` holds the two small checks supporting the ∞-categorical part of the memo (`checks_freeness.py`: the composition product of reduced sequences is orbit-free, Lemma 2.2 of the memo; `checks_F2_pi0.py`: π₀-level cooperad structures on the constant sequence, Remark 3.3 of the memo).
